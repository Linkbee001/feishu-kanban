...
- **Global scope**: `C:/workspace/feishu-kanban/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/workspace/feishu-kanban/.claude/notes/`)
...
2. Glob `C:/workspace/feishu-kanban/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/workspace/feishu-kanban/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/workspace/feishu-kanban/.claude/notes/` — works in any directory
